...
export class DecimalFormatPipe implements PipeTransform {
  transform(value: string, format: string = "###,###.00"): string {
...
function DecimalFormat(formatStr) {
  ...
  DecimalFormat.prototype.format = function(numStr) {
    ...
    if (fracStr.length > maxFrac) {
      // round
      //case 6143
      var num: any = new Number("0." + fracStr);
      ...
      var x: any = intStr.length - 1;
      var i: any = intStr.length - 1;
      while (c) {
        //increment intStr
        if (i == -1) {
          intStr = "1" + intStr;
    ...
    for (var i: any = fracStr.length; i < minFrac; i++) {
    ...
    for (var i: any = intStr.length; i < minInt; i++) {
    ...
    var j = 0;
    for (var i: any = intStr.length; i > 0; i--) {
      // add commas
